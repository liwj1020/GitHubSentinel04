...
class ReportGenerator:
    ...
    def generate_ainewsletter_report(self, markdown_file_path):
        """
        Generate AI Newsletter daily report and save it as {original_filename}_ainewsletter.md.
        """
        with open(markdown_file_path, 'r') as file:
            markdown_content = file.read()

        system_prompt = self.prompts.get("ainewsletter_daily_report")
        report = self.llm.generate_report(system_prompt, markdown_content)
        
        report_file_path = os.path.splitext(markdown_file_path)[0] + "_ainewsletter.md"
        with open(report_file_path, 'w+') as report_file:
            report_file.write(report)

        LOG.info(f"AI Newsletter daily report saved to {report_file_path}")
        return report, report_file_path
