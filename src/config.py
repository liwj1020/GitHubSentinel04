...
class Config:
    ...
    def load_config(self):
        with open('config.json', 'r') as f:
            ...
            self.ollama_model_name = llm_config.get('ollama_model_name', 'llama3.1:8b-instruct-q4_0')
            self.ollama_api_url = llm_config.get('ollama_api_url', 'http://localhost:11434/api/chat')
            
            # 加载报告类型配置
            self.report_types = config.get('report_types', ["github", "hacker_news", "ainewsletter"])  # 默认报告类型
