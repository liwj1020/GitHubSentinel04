...
from ainewsletter_client import AINewsletterClient  # 导入 AI Newsletter 客户端
...
ainewsletter_client = AINewsletterClient()  # 创建 AI Newsletter 客户端实例
...
def generate_ainewsletter_report(model_type, model_name):
    config.llm_model_type = model_type
    ...
    markdown_file_path = ainewsletter_client.export_latest_articles()
    report, report_file_path = report_generator.generate_ainewsletter_report(markdown_file_path)
...
def update_model_list(model_type):
    if model_type == "openai":
        ...
        return gr.Dropdown(choices=["llama3.1:8b-instruct-q4_0", "gemma2:2b", "qwen2:7b"], label="选择模型")
...
with gr.Blocks(title="GitHubSentinel") as demo:
    ...
    # 创建 AI Newsletter 报告 Tab
    with gr.Tab("AI Newsletter 报告"):
        gr.Markdown("## AI Newsletter 报告")  # 添加小标题
        ...
        # 创建按钮来生成报告
        button = gr.Button("生成 AI Newsletter 报告")
        ...
        # 将按钮点击事件与导出函数绑定
        button.click(generate_ainewsletter_report, inputs=[model_type, model_name], outputs=[markdown_output, file_output])
