...
from ainewsletter_client import AINewsletterClient
...
def ainewsletter_job(ainewsletter_client, report_generator, notifier):
    LOG.info("[开始执行定时任务]AI Newsletter 每日报告")
    markdown_file_path = ainewsletter_client.export_latest_articles()
    if markdown_file_path:
        report, _ = report_generator.generate_ainewsletter_report(markdown_file_path)
        notifier.notify_ainewsletter_report(report)
    LOG.info(f"[定时任务执行完毕]")
...
def main():
    ...
    # 安排 AI Newsletter 的定时任务
    schedule.every().day.at("09:00").do(ainewsletter_job, ainewsletter_client, report_generator, notifier)
